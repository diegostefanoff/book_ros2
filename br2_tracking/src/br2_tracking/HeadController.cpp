...
HeadController::control_sycle()
{
  ...
  if (last_command_ == nullptr || (now() - last_command_ts_) > 100ms) {
    ...
    command_msg.points[0].positions[0] = last_state_->output.positions[0] - control_pan;
    command_msg.points[0].positions[1] = last_state_->output.positions[1] - control_tilt;
